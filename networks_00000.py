...
class CombinedCNNLSTM(nn.Module):
    def __init__(self, conf):
        ...
        # CNN layers
        self.conv1 = nn.Conv2d(5, 32, kernel_size=3, stride=1, padding=1)  # Modification ici
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        # LSTM layers
        ...
        self.fc = nn.Linear(conf["LSTM_conf"]["hidden_dim"], 64)  # 64 pour un plateau de jeu 8x8
    ...
    def train_all(self, train_loader, dev_loader, num_epochs, device, optimizer):
        ...
        for epoch in range(num_epochs):
            self.train()
            total_loss = 0
            for data, targets in train_loader:  # Ici, nous nous attendons à deux éléments
                #data, targets = data.to(device), targets.to(device)
                data = data.float().to(device)  # Convertir en float et envoyer au device
                targets = targets.long().to(device)
                # Check if target values are within the expected range
                num_classes = self.fc.out_features
                if (targets < 0).any() or (targets >= num_classes).any():
                    print("Invalid target values. Check the target range.")
                    continue 
    ...
    def evaluate(self, test_loader, device):
        ...
        self.plot_confusion_matrix(conf_matrix, all_targets) 
