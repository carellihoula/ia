...
class CustomDataset(Dataset):
    def __init__(self,
        ...
        if self.load_data_once4all:
            ...
            for gm_idx,gm_name in tqdm(enumerate(self.game_files_name)):
                try:
                    print(f"Trying to open file: {self.path_dataset + gm_name}")
                    h5f = h5py.File(self.path_dataset + gm_name, 'r')
                    game_log = np.array(h5f[gm_name.replace(".h5", "")][:])
                    h5f.close()
                except FileNotFoundError as e:
                    print(f"FileNotFoundError: {e}")
                    # Ajoutez d'autres informations de débogage ici si nécessaire
                    continue  # Passez à la prochaine i
                last_board_state=copy.copy(game_log[0][-1])
                is_black_winner=isBlackWinner(game_log[1][-1],last_board_state)
                for sm_idx in range(30):
                    if is_black_winner:
...
dataset_conf["path_dataset"] = "C:/Users/carel/Documents/IA/deeplearning2playothello-main/dataset/"
...
conf={
    ...
    "LSTM_conf": {
        "hidden_dim": 256,
        "num_layers": 1,  # Ajout de couches LSTM
...
save_models_dir = "save_models_MLP"
